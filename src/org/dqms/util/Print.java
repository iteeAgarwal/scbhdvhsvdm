...
public class Print
{
    ...
    **/
    protected static boolean _includeStackFrame()
    {
        if (printLogIncludeFrame < 0) {
            printLogIncludeFrame = (SystemSetting.LOG_INCL_STACKFRAME || Print.isDebugLoggingLevel())? 1 : 0;
        }
    ...
    **/
    protected static boolean _includeDate()
    {
        if (printLogIncludeDate < 0) {
            printLogIncludeDate = SystemSetting.LOG_INCL_DATE? 1 : 0;
        }
    ...
    **/
    ...
    {
        return SystemSetting.LOG_EMAIL_EXCEPTIONS;
    }
    ...
    **/
    private static PrintStream _getSystemErr()
    {
        if (Print.sysErr == null) {
            try {
                if (SystemSetting.LOG_JAVA_LOGGER) {
                    String n = SystemSetting.LOG_NAME;
    ...
    **/
    public static OutputStream getRedirectedOutputStream()
    {
        if (!Print.logPrintStream_init) {
            ...
            String rosClassName = SystemSetting.LOG_REDIRECT_LOG;
            ...
            if (rosClassName.equalsIgnoreCase("CachedLogOutputStream")) {
                // log.redirectLog=org.dan.util.CachedLogOutputStream
                rosClassName = "org.dan.util.CachedLogOutputStream";
            }
        ...
        if (!Print.printLogFile_init) {
            synchronized (Print.logLock) {
                if (!Print.printLogFile_init) { // check again
                    ...
                    if (!SystemSetting.LOG_FILE_ENABLE) {
                        // log file not enabled
                    } else {
                        // get log file path (if defined)
                        fileStr = SystemSetting.LOG_FILE;
    ...
    **/
    public static long getRotateLogFileSize()
    {
        if (Print.printRotateLogFileSize < 0L) {
            String maxSizeS = SystemSetting.LOG_FILE_ROTATE_SIZE;
            long maxSize = 0L;
    ...
    **/
    public static long getRotateDeleteAgeSec()
    {
        if (Print.printRotateDelAgeSec < 0L) {
            String delAgeStr = SystemSetting.LOG_FILE_ROTATE_DELETE_AGE;
            ...
            try {
                ...
                for (;;) { // single-pass loop
                    ...
                    if ((maxSize > 0L) && logFile.isFile() && (logFile.length() > maxSize)) {
                        ...
                        String rotExtn = Print.formatDate(SystemSetting.LOG_FILE_ROTATE_EXTN);
    ...
    **/
    public static int getLogLevel()
    {
        if (printLogLevel <= LOG_UNDEFINED) {
            printLogLevel = Print.parseLogLevel(SystemSetting.LOG_LEVEL, LOG_INFO);
        }
    ...
    **/
    public static int getLogHeaderLevel()
    {
        if (printLogHeaderLevel <= LOG_UNDEFINED) {
            printLogHeaderLevel = Print.parseLogLevel(SystemSetting.LOG_LEVEL_HEADER, LOG_ALL);
        }
