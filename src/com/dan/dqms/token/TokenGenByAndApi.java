...
@WebServlet("/TokenGenByAndroidApi")
public class TokenGenByAndApi extends HttpServlet {
	private static final long serialVersionUID = 1L;
	HttpSession httpsession;
	HttpServletRequest request;
	HttpServletResponse response;
	public TokenGenByAndApi() {
		super();
	...
	int dep_id=	deptListOb.getDepIdByName(json.getString("depname"));
	List<Room> roomsList =null;
		  if(!json.isNull("depname"))
		  {
			  
		RoomsList roomListOb = new RoomsList();
		roomsList = roomListOb.getRoomsBydepID(String.valueOf(dep_id));
		  }
		  if(!json.isNull("roomno"))
		  {
			String  roomNo=json.getString("roomno");
			int rm= new RoomsList().getRoomsdepID(roomNo);
			roomno=String.valueOf(rm);
		  }
...
/*			if (gettoken != null) {*/
			...
			int rooms_list = Integer.parseInt(roomno);
...
/*				if (allPatList.size() > 0) {
				...
				int tokengen=	ts.insertintotokendetails(ts.getRoomGroup(roomno),
									dep_id, patIDNew, todaytTime, 0, rooms_list,
									user_id,Integer.parseInt(appWalk),value,select_tokenValue);							
							request.setAttribute("emptymess",
		...
		/*	ArrayList<TokenStatusBean> tokenStatus = (ArrayList<TokenStatusBean>) getTokenStatus();
			...
			request.setAttribute("patList", new Patient());
